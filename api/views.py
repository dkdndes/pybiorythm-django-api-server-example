...
from biorhythm_data.models import BiorhythmAnalysis, BiorhythmCalculation, BiorhythmData, Person

from .serializers import (
    ApiInfoSerializer,
    BiorhythmAnalysisSerializer,
    BiorhythmCalculationRequestSerializer,
    BiorhythmCalculationSerializer,
    BiorhythmDataSerializer,
    BiorhythmDataSimpleSerializer,
    PersonSerializer,
...
class PersonViewSet(viewsets.ModelViewSet):
    ...
    def biorhythm_data(self, request, pk=None):
        ...
        return Response(
            {
                "person": {"id": person.id, "name": person.name, "birthdate": person.birthdate},
    ...
    def statistics(self, request, pk=None):
        ...
        stats = data_queryset.aggregate(
            ...
            critical_days=Count(
                "id",
                filter=Q(is_physical_critical=True)
        ...
        return Response(
            {
                ...
                "statistics": {
                    ...
                    "date_range": {
                        ...
                        "end": stats["max_date"],
                        "days_covered": (stats["max_date"] - stats["min_date"]).days + 1
                        if stats["min_date"] and stats["max_date"]
                        else 0,
                    },
                    "cycle_averages": {
                        "physical": round(stats["avg_physical"], 3)
                        if stats["avg_physical"]
                        else None,
                        "emotional": round(stats["avg_emotional"], 3)
                        if stats["avg_emotional"]
                        else None,
                        "intellectual": round(stats["avg_intellectual"], 3)
                        if stats["avg_intellectual"]
                        else None,
                    },
                    "critical_days": {
                        "total": stats["critical_days"],
                        "breakdown": critical_breakdown,
                        "percentage": round((stats["critical_days"] / stats["count"] * 100), 2)
                        if stats["count"] > 0
                        else 0,
                    },
                },
...
def api_info(request):
    ...
    info_data = {
        ...
        "endpoints": {
            "auth": {"token": "/api/auth/token/", "description": "Obtain authentication token"},
            "people": {
                "list": "/api/people/",
...
def global_statistics(request):
    ...
    # Date range
    date_stats = BiorhythmData.objects.aggregate(earliest_date=Min("date"), latest_date=Max("date"))
    ...
    recent_people = Person.objects.filter(created_at__gte=week_ago).count()
    ...
    stats_data = {
        ...
        "date_range": {
            ...
            "span_days": (date_stats["latest_date"] - date_stats["earliest_date"]).days + 1
            if date_stats["earliest_date"] and date_stats["latest_date"]
            else 0,
        },
        "cycle_statistics": {
            "physical": {
                "average": round(cycle_stats["avg_physical"], 3)
                if cycle_stats["avg_physical"]
                else None,
                "min": round(cycle_stats["min_physical"], 3)
                if cycle_stats["min_physical"]
                else None,
                "max": round(cycle_stats["max_physical"], 3)
                if cycle_stats["max_physical"]
                else None,
            },
            "emotional": {
                "average": round(cycle_stats["avg_emotional"], 3)
                if cycle_stats["avg_emotional"]
                else None,
                "min": round(cycle_stats["min_emotional"], 3)
                if cycle_stats["min_emotional"]
                else None,
                "max": round(cycle_stats["max_emotional"], 3)
                if cycle_stats["max_emotional"]
                else None,
            },
            "intellectual": {
                "average": round(cycle_stats["avg_intellectual"], 3)
                if cycle_stats["avg_intellectual"]
                else None,
                "min": round(cycle_stats["min_intellectual"], 3)
                if cycle_stats["min_intellectual"]
                else None,
                "max": round(cycle_stats["max_intellectual"], 3)
                if cycle_stats["max_intellectual"]
                else None,
            },
        },
        "recent_activity": {
            ...
            "average_data_points_per_person": round(total_data_points / total_people, 2)
            if total_people > 0
            else 0,
            "critical_days_percentage": round((total_critical_days / total_data_points * 100), 2)
